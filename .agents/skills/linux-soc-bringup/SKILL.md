...
- For timer failures, separate the Sstc/STIP and SBI TIME/MTIP paths and trace counter, comparator, pending, enables/delegation, trap cause, rearm, and `WFI` wakeup. Require directed boundary tests and 1,000 randomized compare/rearm operations with no missed, duplicate, or early event.
- Bound diagnostic runs with a guest-time and instruction watchdog; stop at the first broken hardware/software boundary and preserve one failing reproducer plus one passing control.
